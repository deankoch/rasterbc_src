...
{
  # download all layers in a loop
  pb = txtProgressBar(min=1, max=length(idx.todownload), style=3)
  for(idx.layer in which(idx.todownload))
  {
    setTxtProgressBar(pb, idx.layer)
    print(paste0('downloading ', unlist(src.filenames)[idx.layer], ' to ', unlist(cfg$src$fname)[idx.layer]))
    download.file(url=unlist(cfg$src$web)[idx.layer], destfile=unlist(cfg$src$fname)[idx.layer], mode='wb') 
  }
  close(pb)
...
# rmarkdown::render(here('src_gfc.R'), run_pandoc=FALSE, clean=TRUE)
# ... or to html ...
# rmarkdown::render(here('src_gfc.R'))
