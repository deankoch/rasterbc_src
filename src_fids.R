...
if(any(idx.todownload))
{
  ...
  # /*
  if(idx.todownload['pre2017'])
  {
    # download the zip to temporary file (?? MB), extract to disk
    fids.tmp = tempfile('fids_temp', cfg$src$dir, '.zip')
    download.file(url=cfg$src$web[['pre2017']], destfile=fids.tmp, mode='wb')
    fids.paths = unzip(fids.tmp, exdir=cfg$src$dir)
    unlink(fids.tmp) 
  }
  idx.todownload = idx.todownload[names(idx.todownload) != 'pre2017']
  
  # download any other missing datasets in a loop
  if(any(idx.todownload))
  {
    pb = txtProgressBar(min=1, max=length(idx.todownload), style=3)
    for(idx.file in which(idx.todownload))
    {
      setTxtProgressBar(pb, idx.file)
      path.elements = unlist(strsplit(unlist(cfg$src$fname)[idx.file+1], '/'))
      print(paste0('downloading ', path.elements[length(path.elements)], ' to ', unlist(cfg$src$fname)[idx.file+1]))
      download.file(url=unlist(cfg$src$web)[idx.file+1], destfile=unlist(cfg$src$fname)[idx.file+1], mode='wb') 
    }
    close(pb)
  } # */
...
# rmarkdown::render(here('src_fids.R'), run_pandoc=FALSE, clean=TRUE)
# ... or to html ...
# rmarkdown::render(here('src_fids.R'))
